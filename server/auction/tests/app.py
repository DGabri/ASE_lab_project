            ...
            for auction in ended_auctions:
                try:
                    ...
                    if auction_complete_data and auction_complete_data.get('winner_id'):
                        if TESTING:
                            logging.info("[TESTING] Updated collection")
                        else:    
                            try:
                                res = requests.post(
...
def create_auction():
    try:
        ...
        # generate error if piece is 100
        if TESTING and piece_id == 100:
            return jsonify({'err': f'Seller: {seller_id} does not have piece: {piece_id}'}), 422
        if not TESTING:
            try:
